...
module.exports = {
  ...
  async execute(cmd, roles) {
    ...
    if (typeof cmd.discriminator === 'string') {user = cmd, channel = await user.createDM();}
    ...
    else {paletteGuilds = paletteGuilds.filter(async guild => await SLAB.findPalette(cmd, guild, user.id));}
